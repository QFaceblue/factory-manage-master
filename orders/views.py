    ...
    if usr.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            if ordermodifyform.is_valid():
                ...
                # ordermodifyform = OrderForm(
                #     initial={'order_name': order_tem.order_name, 'order_client': order_tem.order_client,
    ...
    if usr.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            if orderappendform.is_valid():
                ...
                add.save()
                print("打你妹！")
                # send_mail('Subject here', '有一个新的订单.', DEFAULT_FROM_EMAIL,['517568768@qq.com'],
                #        fail_silently=False)
                return redirect(reverse('order_list'))
            else:
                orderappendform = OrderForm(request.POST)
